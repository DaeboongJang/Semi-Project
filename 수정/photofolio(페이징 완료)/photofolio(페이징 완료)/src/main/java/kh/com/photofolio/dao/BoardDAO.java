...
public class BoardDAO {
	...
	public String selectId(String id) {
		String sql = "select post_id from tbl_post";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
	...
	// id로 게시글 조회
	public BoardDTO selectFromId(String id) {
		String sql = "select * from tbl_post where post_writer=?";
	...
	// 페이징 메서드
	public ArrayList<BoardDTO> getBoardList(int startRange, int endRange, String id) {
		String sql = "select * from" + "(select row_number() over(order by post_no desc) 순위," + "a.* from tbl_post a)"
				+ "where 순위 between ? and ? and post_writer=?";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
	...
	public int getBoardListCnt(String post_writer) { // 게시글 번호로 검색 리스트 갯수
		String sql = "select count(*) from" + "(select row_number() over(order by post_no desc) 순위,"
				+ "a.* from tbl_post a)" + "where post_writer=?";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
	...
	public ArrayList<BoardInfoDTO> getBoardByWriter(String value) throws Exception {
		String sql = "select profilephoto_path,  \r\n"
				...
				+ "post_no, post_writer,post_writer_nickname,post_title, post_content, post_createdDate, post_view_count,category_no,\r\n"
				+ "seq_file, origin_name, system_name\r\n" + "having post_writer_nickname like '%" + value
				+ "%' or post_writer like '%" + value + "%' " + "or post_title like '%" + value
				+ "%' or post_content like '%" + value + "%'" + "order by post_no desc";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			ResultSet rs = pstmt.executeQuery();
	...
	public String selectIdByNo(int post_no) throws Exception {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			...
			ResultSet rs = pstmt.executeQuery();
			if (rs.next())
				return rs.getString("post_writer");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
